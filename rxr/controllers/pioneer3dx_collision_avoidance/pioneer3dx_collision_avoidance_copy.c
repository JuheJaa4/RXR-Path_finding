...
typedef enum { FORWARD, LEFT, RIGHT, DOWNWARD, UPWARD } State;
...
State getNextDirection(WbDeviceTag gps, WbDeviceTag cmps) {
  ...
  for (int i = 0; i < 4; i++) {
    if (wallsFormatted[i] == false) {
      for (int a = 0; a < 4; a++) {
        if (vapaat[a] == NULL) {
          printf("Talletetaan taulukkoon i: %d\n", i);
          vapaat[a] = i;
          break;
        }
      }
    ...
    if (vapaat[i] != NULL) {
      switch (vapaat[i]) {
        case 0:
          ...
          return UPWARD;
        case 1:
          printf("Right taulukosta\n");
          return RIGHT;
        case 2:
          printf("Down taulukosta\n");
          return DOWNWARD;
        case 3:
          printf("Left\n");
          return LEFT;
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    switch (state2) {
      case FORWARD:
        ...
        break;
      case LEFT:
        if ((bearing < 275 && bearing > 265)) {
          printf("Käännytään\n");
          sensorsEnabled = false;
          speed[0] = -0.2 * MAX_SPEED;
          speed[1] = 0.2 * MAX_SPEED;
        } else {
          printf("Ei tarvitse enää kääntyä\n");
          sensorsEnabled = true;
          state2 = FORWARD;
          speed[0] = MAX_SPEED;
          speed[1] = MAX_SPEED;
